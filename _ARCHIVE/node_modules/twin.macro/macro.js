...
var get = function (object, path$$1, defaultValue) {
  var travel = function (regexp) { return String.prototype.split.call(path$$1, regexp).filter(Boolean).reduce(function (result, key) { return result !== null && result !== undefined ? result[key] : result; }, object); };
...
var normalizeDynamicConfig = function (ref) {
  ...
  value: ("" + value)}));
  }).filter(function (item) { return !item.target.includes('-array-') && (typeof item.rating === 'undefined' || item.rating >= 0.15); });
};
...
var addDataTwPropToPath = function (ref) {
  ...
  if (state.isProd || !state.configTwin.dataTwProp) { return; } // Remove the existing debug attribute if you happen to have it

  var dataTwProperty = attributes.filter(function (p) { return p.node && p.node.name && p.node.name.name === 'data-tw'; });
  dataTwProperty.forEach(function (path$$1) { return path$$1.remove(); }); // Add the attribute
...
var addDataTwPropToExistingPath = function (ref) {
  ...
  if (state.isProd || !state.configTwin.dataTwProp) { return; } // Append to the existing debug attribute

  var dataTwProperty = attributes.find( // TODO: Use @babel/plugin-proposal-optional-chaining
...
function findRightBracket(classes, start, end) {
...
function spreadVariantGroups(classes, context, importantContext, start, end) {
  ...
  if (classes === '') { return []; }
  var results = [];
  classes = classes.slice(start, end).trim(); // variant format: /[\w-]+:/
  // class format: /[\w-./]+/
  ...
  while (match = reg.exec(classes)) {
    ...
    if (variant) {
      ...
      var closeBracket$1 = findRightBracket(classes, match.index);
      ...
      results.push.apply(results, spreadVariantGroups(classes, context, importantContext || importantGroup$1, match.index + 1, closeBracket$1));
...
var matchChildKey = function (from, matcher) {
  ...
  var loop = function () {
    ...
    if (isEmpty(splitMatcher[1]) || !splitMatcher[1].startsWith('-')) { return; }
    ...
    throwIf(!isValueReturnable, function () { return logGeneralError(("The tailwind config is nested too deep\nReplace this with a string, number or array:\n" + (JSON.stringify(objectMatch)))); });
...
var getStyleData = (function (classes, t, state, silentMismatches) {
  if ( silentMismatches === void 0 ) silentMismatches = false;
  ...
  var styles = classesOrdered.reduce(function (results, classNameRaw) {
    ...
    !silentMismatches && doPrechecks([precheckGroup], {
...
var handleTwProperty = function (ref) {
  ...
  var ref$1 = getStyleData(rawClasses, t, state);
  ...
  var attributeList = attributes.map(function (p) { return p.node.name && p.node.name.name; });
  ...
  if (expr.isArrayExpression()) {
    ...
    throwIf(!cssProperty, function () { return logGeneralError("An empty css prop (css=\"\") isn’t supported alongside the tw prop (tw=\"...\")"); });
    expr.replaceWith(t.arrayExpression(hasCssFirst ? [cssProperty, styles] : [styles, cssProperty]));
  }
...
var handleTwFunction = function (ref) {
  ...
  defaultImportReferences.forEach(function (path$$1) {
    ...
    var ref = getStyleData(rawClasses, t, state);
    var styles = ref.styles;
...
var handleClassNameProperty = function (ref) {
  ...
  var ref$1 = getStyleData(rawClasses, t, state, true);
  var styles = ref$1.styles;
  ...
  var cssExpression = cssAttributes[0].get('value').get('expression');
  var attributeNames = getAttributeNames(jsxPath);
  var isBeforeTwAttribute = attributeNames.indexOf('className') - attributeNames.indexOf('tw') < 0;
  ...
  if (cssExpression.isArrayExpression()) {
    ...
    isBeforeTwAttribute || isBeforeCssAttribute ? cssExpression.unshiftContainer('elements', styles) : cssExpression.pushContainer('elements', styles);
  } else {
    ...
    var styleArray = isBeforeTwAttribute || isBeforeCssAttribute ? [styles, existingCssAttribute] : [existingCssAttribute, styles];
...
var getBuiltRules = function (rule, isBase) {
  ...
  var selectorParsed = selectorArray.map(function (s) { return parseSelector(s, isBase); }).filter(Boolean); // Join them back into a string
...
var getUserPluginRules = function (rules, screens, isBase) { return rules.reduce(function (result, rule) {
  var obj;
...
var twinMacro = function (ref) {
  ...
  program.traverse({
    ...
    JSXAttribute: function JSXAttribute(path$$1) {
      if (path$$1.node.name.name === 'css') { state.hasCssProp = true; }
  ...
  if ((state.hasTwProp || state.hasCssProp) && configTwin.autoCssProp === true && state.isStyledComponents) {
    maybeAddCssProperty({
      program: program,
      t: t
    });
